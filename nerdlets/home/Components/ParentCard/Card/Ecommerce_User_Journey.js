...
const Ecommerce_User = ({ cardName, timeUpdater, guid, colorSetter }) => {
        ...
        `);
            ...
            query {
              actor {
                ...
                account(id: 2781667) {
                  nrql(
                    query: "FROM WorkloadStatus SELECT latest(statusValue) WHERE entity.name IN (${dict.infra_workloads.map(infra => `'${infra}'`).join(',')})"
                  ) {
                    embeddedChartUrl
                    nrql
                    otherResult
          ...
          `)
                ...
                account(id: 2781667) {
                nrql(
                        query: "SELECT count(*) from NrAiIncident where event = 'open' and priority = 'critical' SINCE ${timeUpdater} AGO COMPARE WITH ${timeUpdater} AGO"
                      ) {
                        embeddedChartUrl
                        nrql
                        otherResult
          ...
          `)
                ...
                const warningAlertCountPromise = fetchNerdGraphQuery(`{
                    ...
                    account(id: 2781667) {
                    nrql(
                            query: "SELECT count(*) from NrAiIncident where event = 'open' and priority = 'warning' SINCE ${timeUpdater} AGO COMPARE WITH ${timeUpdater} AGO"
                          ) {
                            embeddedChartUrl
                            nrql
                            otherResult
            ...
            actor {
              account(id: 2781667) {
                ...
                setUserData_Custom(prevState => ({
                    ...
                    'End user experience': {
                        current: fetch_user_performance.actor.account.nrql.rawResponse.current.results[0].score,
                        prev: fetch_user_performance.actor.account.nrql.rawResponse.previous.results[0].score
                    },
    ...
    const fetch_end_userPerformance_Promise = async (appName) => {
        try {
          ...
          actor {
            account(id: 2781667) {
              nrql(query: "FROM BrowserInteraction SELECT apdex(duration) WHERE appName = '${appName}' SINCE ${timeUpdater} AGO COMPARE WITH ${timeUpdater} AGO") {
                embeddedChartUrl
                nrql
                otherResult
                rawResponse
    ...
    const fetch_login_custom_func = async (appName, dynamicURI) => {
        try {
          ...
          actor {
            account(id: 2781667) {
              nrql(query: "FROM Transaction SELECT average(duration) WHERE appName = '${appName}' AND request.uri IN ('${dynamicURI}') SINCE ${timeUpdater} AGO COMPARE WITH ${timeUpdater} AGO") {
                embeddedChartUrl
                nrql
                otherResult
                rawResponse
                ...
                <Card.Body>
                    ...
                    <Card.Text>
                        <ListGroup variant="flush">
                            ...
                            <ListGroup.Item className="listItemCustomPadding">
                                <Row>
                                    <Col sm={7}>End User Experience  </Col>
                                    <Col sm={1}>
                                        <FontAwesomeIcon
                                            icon={
                            ...
                            <ListGroup.Item className="listItemCustomPadding">
                                <Row>
                                    <Col sm={7}>Partner Authetication RT (sec)</Col>
                                    <Col sm={1}>
                                        <FontAwesomeIcon
                                            icon={
